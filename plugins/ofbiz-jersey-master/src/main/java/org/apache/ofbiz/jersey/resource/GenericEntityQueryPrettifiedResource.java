...
import org.apache.ofbiz.jersey.pojo.EntityQueryInputPrettified;
...
@Path("/generic/v1/entityqueryprettified/")
@Provider
@Secured
public class GenericEntityQueryPrettifiedResource {
	...
	public Response queryEntities(@PathParam(value = "name") String entityName, String jsonBody) {
		...
		EntityQueryInputPrettified query;
		try {
			query = mapper.readValue(jsonBody, EntityQueryInputPrettified.class);
		...
		try {
			conditionList = getConditionList(entityName, UtilMisc.toMap("inputFields", convertToOldFormat(query.getInputFields())));
	...
	private List<Map<String, ?>> recursiveFind(EntityQueryInputPrettified queryInput, String entityName, Delegator delegator, List<GenericValue> values) throws GenericServiceException, GenericEntityException {
		List<Map<String, ?>> queryResponse = new ArrayList<>();
		EntityConditionList<EntityCondition> conditionList = getConditionList(entityName, UtilMisc.toMap("inputFields", convertToOldFormat(queryInput.getInputFields())));
		...
		for (GenericValue val : values) {
			if (conditionList == null || conditionList.entityMatches(val)) {
				...
				for (String singularRel : singularRelationName) {
					...
					EntityQueryInputPrettified relQueryInput = queryInput.getEntityRelations().get(fullName);
	...
	private Map<String, Object> convertToOldFormat(List<Map<String, Object>> params) {
		...
		return performFindInputFieldsInput;
	}
}
